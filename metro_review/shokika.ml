...
let shokika lst siten = 
  List.map (fun ({namae = n; saitan_kyori = s; temae_list = t} as x) ->
      if n = siten then {namae = n; saitan_kyori = 0.; temae_list = [siten]}
      else x) lst


(* test *)
let test1 = shokika (make_eki_list global_ekimei_list) "代々木公園" 
